...
import ipcApi from './apis/ipcApi';
...
app.on( 'ready', () => {
    ...
    let mainWindow = new BrowserWindow({
        title: 'Futures Predictive System'
    })
    ...
    // // 監聽前端 API
    // ipcMain.on('callApi', (event, apiName, ...args) => {
        
    //     let apiPathAry = apiName.split('.');
    //     let func = Api;

    //     apiPathAry.map(apiPath => {
            
    //         if(!func[apiPath]) func = func['default'];

    //         func = func[apiPath];
    //     })

    //     event.returnValue = (func)? func(...args): null;
